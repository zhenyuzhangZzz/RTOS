...
int systick_open(device_t *dev)
{
	printk("open sistick device!\n");
	return 0;
}

struct drv_operations systick_ops = {
	.open = systick_open,
};

/*
* systick_init() 初始化system tick定时器,定时10ms
*/
...
*中断回调函数
*/
void systick_handle(uint_t irq, void *data)
{
...
drvstus_t systick_drv_init(void)
{
	device_t *dev;
	driver_t *drv;
	dev_t dev_id;
	...
	/*1、分配driver结构*/
	drv = driver_alloc("systick", &systick_ops);

	if(drv == NULL)
		return DFCERRSTUS;

	/*2、注册字符设备并返回设备号和device*/
	dev_id = register_chrdev(0, &dev, "systick");
	...
	request_irq(IRQ_SYSTIMER, systick_handle, dev, IRQF_TRIGGER_NONE, "systick", NULL);
